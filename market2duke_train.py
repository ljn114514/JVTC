...
src_dir = dataset_path + 'Market-1501/bounding_box_train/'
tar_dir = dataset_path + 'DukeMTMC-reID/bounding_box_train_camstyle_merge/'
tar_dir_test = dataset_path + 'DukeMTMC-reID/'

src_annfile = 'list_market/list_market_train.txt'
tar_annfile = 'list_duke/list_duke_train.txt'
tar_annfile_test = 'list_duke/list_duke_test.txt'
...
###########   MODEL   ###########
model, param = resnet50(pretrained=imageNet_pretrain, num_classes=751)
model.cuda()
...
print('evaluation..')
model.eval()
evaluate_all(model, test_loader_t, select_set='duke')
		
if hasattr(model, 'module'):
	model = model.module
torch.save(model.state_dict(), 'snapshot/resnet50_market2duke_epoch%05d.pth'%(epoch))	
print('save snapshot:','snapshot/resnet50_market2duke_epoch%05d.pth'%(epoch))